...
export class RevenueEntities1765353832837 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`revenue_realizations\` (\`id\` varchar(36) NOT NULL, \`company_id\` int NOT NULL, \`date\` date NOT NULL, \`amount\` decimal(15,2) NOT NULL, \`description\` text NULL, \`user_id\` varchar(36) NULL, \`createdAt\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updatedAt\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, UNIQUE INDEX \`IDX_6bfe65d4d794bd7249a7e74534\` (\`company_id\`, \`date\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`revenue_targets\` (\`id\` varchar(36) NOT NULL, \`company_id\` int NOT NULL, \`year\` int NOT NULL, \`month\` int NOT NULL, \`targetAmount\` decimal(15,2) NOT NULL, \`createdAt\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updatedAt\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, UNIQUE INDEX \`IDX_ee493813bbeaf8c9f488e288cc\` (\`company_id\`, \`year\`, \`month\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`users\` DROP FOREIGN KEY \`FK_a2cecd1a3531c0b041e29ba46e1\``);
        await queryRunner.query(`ALTER TABLE \`refresh_tokens\` DROP COLUMN \`created_at\``);
        await queryRunner.query(`ALTER TABLE \`refresh_tokens\` ADD \`created_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP`);
        await queryRunner.query(`ALTER TABLE \`refresh_tokens\` DROP COLUMN \`token_hash\``);
